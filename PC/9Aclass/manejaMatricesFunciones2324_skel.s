...
main:
    ...
    li $t0, 2       #Fila del elemento a cambiar
    li $t1, 3       #Columna del elemento a cambiar


    #Acceder a la direccion de memoria de la matriz 1
    la $s0, mat1
    #Como sacar el numero de filas
    lw $s1, 0($s0)      #Ese 0 podemos ponerlo como nFil y para eso nos colocaron eso arriba los profes
    #Como sacar el numero de columnas
    lw $s2, 4($s0)      #Ese 0 podemos ponerlo como nCol y para eso nos colocaron eso arriba los profes
    # Como le sume 8, ahora tendra la direccion de memoria del primer elemento de la matriz (Porque es un 8) 
    addi $s3, $s0, 8    # Como antes, si uso la var "elementos" de arriba, pos tendremos lo mismo
    # Para cambiar el contenido del primer elemento a un -5.2 por ejemplo

...
// Imprime una matriz por la consola 
void print_mat(structMat* mat) {
  int nFil = mat->nFil;
  int nCol = mat->nCol;
  // elem es la direccion de memoria donde estan los datos reales de la matriz
  float* elem = mat->elementos;
  std::cout << "\n\nLa matriz tiene dimension " << nFil << 'x' << nCol << '\n';
  for(int f = 0; f < nFil; f++) {
    for(int c = 0; c < nCol; c++) {
      // La fórmula de la tutoría academica nº 5 que nos ayuda con la posicion exacta de cierto elemento en la matriz tan solo con la i y la j
